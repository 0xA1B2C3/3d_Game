...
void UpdateKeyStates() {
	...
	if (GetAsyncKeyState('W')) {
		CG->CurLevel->cam.poswrldz += cos(heading*piover180) * 0.05f;
		CG->CurLevel->cam.poswrldx += sin(heading*piover180) * 0.05f;
	}
	...
	if ((global_pos.x != global_x / 2) || (global_pos.y != global_y / 2)) {
		if (global_pos.x > global_x / 2) {//Right?
			//CG->CurLevel->cam.rotwrldy -= (global_y / 2 - global_pos.y) * 0.01;
			heading -= .5;
		}
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glRotated(360 - heading2, 1.0, 0.0, 0.0);
